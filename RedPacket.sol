...
contract RedPacket is Pausable, SafeMath {
    ...
    struct Promotion {
        ...
        uint eachRedPacketAmt; // the amount of reward in each red pocket. Unit in msg.value / wei
        uint maxRedPacketNum; 
        uint claimedNum;
        uint moneyPool;
        ...
        returns (uint)
    {
        ...
        Promotion memory _promotion = Promotion({
            ...
            msg: _msg,
            url: _url,
            eachRedPacketAmt: safeDiv(deposit, _maxNum),
            maxRedPacketNum: _maxNum,
            claimedNum: 0,
            moneyPool: deposit,
    ...
    function claimReward(uint _promoteID, uint _moneyPool) whenNotPaused {
        ...
        // send red pocket
        if (msg.sender.send(p.eachRedPacketAmt)) {
            p.moneyPool -= p.eachRedPacketAmt;
            p.claimedNum++;
        ...
        if (p.moneyPool < p.eachRedPacketAmt || p.claimedNum >= p.maxRedPacketNum || (block.number - p.startBlock >= p.blockLast)) {
            p.finished = true;
    ...
    function getPromotion(uint256 _id)
        ...
        msg = p.msg;
        url = p.url;
        eachRedPacketAmt = p.eachRedPacketAmt;
        maxRedPacketNum = p.maxRedPacketNum;
